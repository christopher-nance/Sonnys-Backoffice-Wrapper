...
- **Hierarchical site restriction.** `employee[disabledRegions][]=<regionId>` means that region is **FULLY GRANTED** (all its sites allowed) — it is *not* an exclusion flag. Likewise `employee[disabledDistricts][]=<districtId>` = district fully granted. Verified live on WashU (create → read the stored access back) and byte-matched against 59 real restricted employees. Correct encoding: (1) a region/district whose sites are **all** granted → emit its `disabled…[]` flag + each of its sites as `[isAvailable]`; (2) any region/district that is only **partially** granted → walk down to per-site and list **every** site once: granted → `[isAvailable]=N`, denied → `[siteId]=N` (a denied site must still be listed with `siteId`, even when its whole region is denied); (3) omit the `isAll*` rollups so Symfony binds them false (`available_sites="all"` uses `isAllRegionsAllowed=1`). Three encodings shipped and each broke production: "submit only the complement's `siteId`" left an untouched region's rollup true and leaked its district; "`siteId` for every site + `isAvailable` for granted" sent both fields for granted sites → grant-all; **"`disabledRegions` for regions with no grant" inverted the flag → the server granted every store in every other region** (the "all sites except the one they need" bug). Validate any change with a live round-trip, not just a `FormData` capture. Flat tenants use the `employee[siteIds][]` blocklist (unverified — no flat tenant available).
