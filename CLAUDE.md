...
- **Hierarchical site restriction uses inverted-looking controls.** On the live WashU edit form, checked `employee[disabledRegions][]`, `employee[disabledDistricts][]`, and per-site `[isAvailable]` controls are labeled **No**. Correct restricted encoding: (1) emit `disabledRegions[]` / `disabledDistricts[]` for groups with no granted site; (2) granted site → enabled hidden `[siteId]=N`; (3) denied site → checked `[isAvailable]=N`; (4) never send both fields for one site; (5) omit restricted `isAll*` rollups (`available_sites="all"` uses `isAllRegionsAllowed=1`). Employee 54's captured edit form is the regression source of truth: it is Fiesta-only, with Fiesta's `siteId` enabled and all other `isAvailable` controls checked. `create_employee` also reads hierarchical access back before applying permissions and fails closed on a mismatch. Flat tenants keep the `employee[siteIds][]` blocklist (unverified — no flat tenant available).
